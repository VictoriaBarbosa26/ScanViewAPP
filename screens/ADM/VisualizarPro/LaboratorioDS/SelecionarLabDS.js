...
export default function MainScreen({ navigation, route }) {
  ...
  const laboratoriosFixos = ['Laboratório 1', 'Laboratório 2', 'Laboratório 3', 'Laboratório 4', 'Laboratório 5'];
  ...
  const handleLabSelect = (laboratorio) => {
    ...
    if (problemas && problemas.length > 0) {
      ...
      navigation.navigate('VisualizaProDS', {
        userID,
    ...
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#0056b3', '#000000']} style={styles.boxesContainer}>
        ...
        <View style={styles.columnContainer}>
          {/* Container para os três primeiros laboratórios */}
          <View style={styles.topRowContainer}>
            {laboratoriosFixos.slice(0, 3).map((laboratorio) => {
              return (
          ...
          {/* Container para os laboratórios 4 e 5 juntos */}
          <View style={styles.bottomRowContainer}>
            {laboratoriosFixos.slice(3).map((laboratorio) => {
              return (
                <View key={laboratorio} style={styles.boxContainerBottom}>
                  <Text style={styles.boxText}>{laboratorio}</Text>
                  <TouchableOpacity
                    style={styles.box}
                    onPress={() => handleLabSelect(laboratorio)}
...
const styles = StyleSheet.create({
  ...
  bottomRowContainer: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    width: '60%', // Reduzindo a largura para juntar as caixas 4 e 5
    marginTop: 20, // Espaço extra para descer as caixinhas 4 e 5
  },
  boxContainerTop: {
    alignItems: 'center',
  ...
  boxContainerBottom: {
    alignItems: 'center',
    width: '40%', // Ajustando a largura para que os laboratórios 4 e 5 fiquem juntos
    marginHorizontal: 1,
  },
  box: {
    backgroundColor: '#fff',
  ...
  spacing: {
    height: 10, // Espaço extra entre o título e os laboratórios
  },
});
